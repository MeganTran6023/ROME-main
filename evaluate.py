...
def mend_apply(model, tok, request, hparams, copy=False, **kwargs):
    """
    Wrapper for MEND that removes unsupported keyword arguments.
    MEND does NOT support:
      - return_orig_weights
      - return_orig_weights_device
    """
    kwargs.pop("return_orig_weights", None)
    kwargs.pop("return_orig_weights_device", None)

    return MendRewriteExecutor().apply_to_model(
        model,
        tok,
        request,
        hparams,
...
ALG_DICT = {
    ...
    "MEND": (MENDHyperParams, mend_apply),
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Run only probability-based tests.",
    )
    parser.add_argument(
        "--conserve_memory",
        action="store_true",
        help="Backup model weights on CPU instead of GPU.",
    )
